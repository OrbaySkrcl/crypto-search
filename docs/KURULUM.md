...
| **Bağlantı testi "hiçbir giriş biçimi sonuç vermedi" diyor** | Aktörün kiralama/deneme süresi dolmuş olabilir — bazı ücretli aktörler hata vermeden boş döner | `APIFY_ACTOR=apidojo/twitter-scraper-lite` yap (varsayılan bu). Tutmazsa `kaitoeasyapi/twitter-x-data-tweet-scraper` dene. Teşhis ekranı aktörün kendi logunu da gösterir. |
